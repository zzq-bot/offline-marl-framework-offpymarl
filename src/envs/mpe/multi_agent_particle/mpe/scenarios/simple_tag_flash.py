...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.size = 0.13 if agent.adversary else 0.05
            agent.accel = 3.0 if agent.adversary else 9.0
            agent.max_speed = 1.0 if agent.adversary else 3.0
        # add landmarks
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            if agent.adversary:
                if i == 0:
                    agent.state.p_pos = np.array([np.random.uniform(-0.5, +0.5, 1), np.random.uniform(+1, +1.5, 1)]).reshape(-1)
                if i == 1:
                    agent.state.p_pos = np.array([np.random.uniform(+1, +1.5, 1), np.random.uniform(-1.5, -1, 1)]).reshape(-1)
                if i == 2:
                    agent.state.p_pos = np.random.uniform(-1.5, -1, world.dim_p)
            else:
        ...
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                if i == 0:
                    ...
                    landmark.state.p_pos = np.array([-1.3, -0.35])
                if i == 1:
                    #landmark.state.p_pos = np.array([1.0, -0.3])
                    landmark.state.p_pos = np.array([-1.3, +0.35])
