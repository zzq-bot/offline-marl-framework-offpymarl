...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.size = 0.09 + EPS if agent.adversary else 0.05 + EPS
            agent.accel = 3.5 + EPS if agent.adversary else 2.4 + EPS
            #agent.accel = 20.0 if agent.adversary else 25.0
            agent.max_speed = 1.3 + EPS if agent.adversary else 0.8 + EPS
        # add landmarks
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                if i == 0:
                    landmark.state.p_pos = np.array([0.5 + EPS, -0.5 + EPS])
                if i == 1:
                    landmark.state.p_pos = np.array([-0.5 + EPS, 0.5 + EPS])
